...
describe("PredictionMarket", function () {
  ...
  describe("Market Creation", function () {
    ...
    it("Should increment market counter", async function () {
      ...
      await predictionMarket.connect(user1).createMarket("Market 1", "Desc", expiresAt, 1);
      await predictionMarket.connect(user1).createMarket("Market 2", "Desc", expiresAt, 1);
      await predictionMarket.connect(user1).createMarket("Market 3", "Desc", expiresAt, 1);
  ...
  describe("Commit-Reveal Betting", function () {
    ...
    it("Should commit a bet successfully", async function () {
      ...
      const tx = await predictionMarket.connect(user1).commitBet(marketId, commitHash, {
        value: TYPICAL_BET,
      });
      ...
      const commitment = await predictionMarket.getCommitment(marketId, user1.address);
    ...
    it("Should fail to commit with insufficient amount", async function () {
      ...
      await expect(
        predictionMarket.connect(user1).commitBet(marketId, commitHash, {
    ...
    it("Should fail to commit twice", async function () {
      ...
      await predictionMarket.connect(user1).commitBet(marketId, commitHash, {
        value: TYPICAL_BET,
      });
    ...
    it("Should reveal bet successfully", async function () {
      ...
      // Reveal
      const tx = await predictionMarket.connect(user1).revealBet(marketId, outcome, salt);
    ...
    it("Should fail to reveal with wrong salt", async function () {
      ...
      await expect(
        predictionMarket.connect(user1).revealBet(marketId, outcome, wrongSalt)
    ...
    it("Should fail to reveal with wrong outcome", async function () {
      ...
      await expect(
        predictionMarket.connect(user1).revealBet(marketId, !outcome, salt)
      ).to.be.revertedWith("Invalid reveal");
    });
    ...
    it("Should update market pools after reveal", async function () {
      ...
      await predictionMarket.connect(user1).revealBet(marketId, outcome, salt);
  ...
  describe("Market Resolution", function () {
    ...
    beforeEach(async function () {
      ...
      await predictionMarket.connect(user1).commitBet(marketId, commitHash1, {
        value: TYPICAL_BET,
      });
      await predictionMarket.connect(user2).commitBet(marketId, commitHash2, {
        value: TYPICAL_BET,
      });

      await predictionMarket.connect(user1).revealBet(marketId, outcome1, salt1);
      await predictionMarket.connect(user2).revealBet(marketId, outcome2, salt2);
    });
    ...
    it("Should resolve market successfully", async function () {
      ...
      const tx = await predictionMarket.connect(resolver).resolveMarket(marketId, outcome, reasoning);
    ...
    it("Should fail to resolve before expiration", async function () {
      await expect(
        predictionMarket.connect(resolver).resolveMarket(marketId, true, "reasoning")
  ...
  describe("Claiming Winnings", function () {
    ...
    beforeEach(async function () {
      ...
      await time.increaseTo(expiresAt + 1);
      await predictionMarket.connect(resolver).resolveMarket(marketId, true, "YES wins");
    });
    ...
    it("Should allow winner to claim winnings", async function () {
      ...
      expect(balanceAfter + gasUsed).to.be.closeTo(balanceBefore + expectedPayout, ethers.parseEther("0.0001"));
  ...
  describe("Admin Functions", function () {
    it("Should allow owner to pause contract", async function () {
      ...
      const expiresAt = (await time.latest()) + 86400;
      await expect(
        predictionMarket.connect(user1).createMarket("Test", "Desc", expiresAt, 1)
    ...
    it("Should allow owner to change resolver", async function () {
      await predictionMarket.connect(owner).setResolverAddress(user3.address);
      expect(await predictionMarket.resolverAddress()).to.equal(user3.address);
    });

    it("Should allow owner to change vault", async function () {
      await predictionMarket.connect(owner).setVaultAddress(user3.address);
      expect(await predictionMarket.vaultAddress()).to.equal(user3.address);
    });
