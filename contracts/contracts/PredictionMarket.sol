...
contract PredictionMarket is Ownable, ReentrancyGuard, Pausable {
    ...
    modifier marketActive(uint256 marketId) {
        ...
        require(
            block.timestamp < markets[marketId].expiresAt,
            'Market expired'
        );
        _;
    }
        ...
        marketActive(marketId)
    {
        ...
        require(
            commitments[marketId][msg.sender].commitHash == bytes32(0),
            'Already committed'
        );
    ...
    function claimWinnings(
        ...
        uint256 grossPayout = (bet.shares * market.totalPool) /
            totalWinningShares;
    ...
    function claimRefund(
        ...
        require(
            market.status == MarketStatus.Cancelled ||
                (market.status == MarketStatus.Resolving &&
                    block.timestamp > market.expiresAt + COMMIT_REVEAL_TIMEOUT),
