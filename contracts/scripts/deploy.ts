...
async function main() {
  ...
  const networkName = network.chainId === 97n ? "bscTestnet" : 
                      network.chainId === 56n ? "bscMainnet" : 
                      network.chainId === 1337n ? "localhost" : 
                      "unknown";
  ...
  console.log(`      npx hardhat verify --network ${networkName} ${predictionMarketAddress} ${vaultAddress} ${deployer.address}`);
