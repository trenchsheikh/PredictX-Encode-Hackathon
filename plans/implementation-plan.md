...
class BlockchainService {
  ...
  constructor() {
    ...
    this.wallet = new ethers.Wallet(process.env.RESOLVER_PRIVATE_KEY!, this.provider);
  ...
  setupEventListeners() {
    ...
    // BetPlaced event
    this.predictionMarket.on('BetPlaced', async (marketId, user, outcome, amount, shares, price, event) => {
      console.log('BetPlaced event:', { marketId, user, outcome, amount });
      await this.handleBetPlaced(marketId, user, outcome, amount, shares, price, event);
    });
    
    // MarketResolved event
    this.predictionMarket.on('MarketResolved', async (marketId, outcome, reasoning, totalPool, event) => {
      console.log('MarketResolved event:', { marketId, outcome });
      await this.handleMarketResolved(marketId, outcome, reasoning, totalPool, event);
    });
    
    // WinningsClaimed event
    this.predictionMarket.on('WinningsClaimed', async (marketId, user, amount, fee, event) => {
  ...
  async createMarket(title: string, description: string, expiresAt: number, category: number, initialOutcome: boolean, value: bigint) {
  ...
  async resolveMarket(marketId: number, outcome: boolean, reasoning: string) {
    const tx = await this.predictionMarket.resolveMarket(marketId, outcome, reasoning);
    const receipt = await tx.wait();
    return receipt;
  }
...
class ResolutionService {
  ...
  private async analyzeWithAI(prediction: any, evidence: any[]): Promise<ResolutionResult> {
...
Be objective, thorough, and base your decision solely on the evidence provided.`;
  ...
  private parseResolutionResponse(response: string, evidence: any[]): ResolutionResult {
    const jsonMatch = response.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
...
class EvidenceService {
  ...
  private async gatherSportsEvidence(prediction: any): Promise<EvidenceSource[]> {
    const evidence: EvidenceSource[] = [];
    ...
    try {
      ...
      evidence.push({ source: 'TheSportsDB', type: 'sports_result', data: sportsDbData });
    } catch (err) {
    ...
    try {
      ...
      evidence.push({ source: 'SportsData.io', type: 'sports_stats', data: sportsDataIo });
    } catch (err) {
  ...
  private async gatherCryptoEvidence(prediction: any): Promise<EvidenceSource[]> {
    const evidence: EvidenceSource[] = [];
    
    try {
      // CoinGecko
      const coinGeckoData = await axios.get('https://api.coingecko.com/api/v3/simple/price', {
        params: {
      ...
      evidence.push({ source: 'CoinGecko', type: 'crypto_price', data: coinGeckoData.data });
    } catch (err) {
    ...
    try {
      ...
      evidence.push({ source: 'Binance', type: 'crypto_price', data: binanceData.data });
    } catch (err) {
    ...
    try {
      ...
      const cmcData = await axios.get('https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest', {
        headers: {
      ...
      evidence.push({ source: 'CoinMarketCap', type: 'crypto_price', data: cmcData.data });
    } catch (err) {
  ...
  private async gatherPoliticsEvidence(prediction: any): Promise<EvidenceSource[]> {
    const evidence: EvidenceSource[] = [];
    ...
    try {
      ...
      evidence.push({ source: 'NewsAPI', type: 'news_articles', data: newsData.data.articles.slice(0, 5) });
    } catch (err) {
  ...
  private async gatherWeatherEvidence(prediction: any): Promise<EvidenceSource[]> {
    const evidence: EvidenceSource[] = [];
    ...
    try {
      ...
      evidence.push({ source: 'OpenWeatherMap', type: 'weather_data', data: weatherData.data });
    } catch (err) {
  ...
  private async gatherGeneralEvidence(prediction: any): Promise<EvidenceSource[]> {
    const evidence: EvidenceSource[] = [];
    ...
    try {
      ...
      evidence.push({ source: 'Google News', type: 'news_articles', data: newsData.data.articles });
    } catch (err) {
...
const predictionSchema = new mongoose.Schema({
  ...
  category: { 
    type: String, 
    enum: ['sports', 'crypto', 'politics', 'entertainment', 'weather', 'finance', 'technology', 'custom'],
    required: true,
    index: true
  },
  status: {
  ...
  try {
    ...
    for (const market of expiredMarkets) {
      try {
        ...
        logger.info(`Market ${market.id} resolved: ${result.outcome} (confidence: ${result.confidence})`);
      } catch (error) {
...
describe('PredictionMarket', function () {
  ...
  beforeEach(async function () {
    ...
    predictionMarket = await PredictionMarket.deploy(vault.address, resolver.address);
  ...
  describe('Betting', function () {
    ...
    it('Should allow placing a bet', async function () {
      ...
      const tx = await predictionMarket.connect(user2).placeBet(marketId, false, { value: amount });
      const receipt = await tx.wait();
    ...
    it('Should update market pools correctly', async function () {
      ...
      await predictionMarket.connect(user2).placeBet(marketId, false, { value: amount });
  ...
  describe('Resolution', function () {
    ...
    it('Should resolve market correctly', async function () {
      const tx = await predictionMarket.connect(resolver).resolveMarket(
        marketId,
        true,
        'BTC reached $100k'
      );
    ...
    it('Should only allow resolver to resolve', async function () {
      await expect(
        predictionMarket.connect(user1).resolveMarket(marketId, true, 'Reasoning')
...
describe('Predictions API', () => {
  ...
  describe('GET /api/predictions', () => {
    ...
    it('should filter by category', async () => {
      ...
      expect(response.body.predictions.every((p: any) => p.category === 'crypto')).toBe(true);
    });
  });
...
*(Can be reduced significantly if building in-house)*
